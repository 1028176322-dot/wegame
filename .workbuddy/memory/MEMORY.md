...
## 记忆类动作豁免边界（Decision Owner 裁定 2026-07-18）
- **记忆沉淀类动作（更新 `.workbuddy/memory/MEMORY.md` 与每日日志 `YYYY-MM-DD.md`）整体豁免三件套**（不需 §21.16 启动回执 / TASK 卡 / CL）。
- 定位：记忆写入是每个实质任务收尾的**附属沉淀动作**，随所属任务一并处理，不自举独立文书。
- 边界：此豁免**仅限**记忆文件本身。凡产生**项目交付物/业务或治理 canonical 改动**的任务（如美术清单、检查器、规则文档改写），仍须完整走三件套，不受此豁免影响。
- 外部动作（commit / push）仍每次单独授权，不因记忆豁免而免除。
- 依据：结案 `TASK-2026-0717-MEM`（DO 边界裁定）。
