...
> 归属域：B 元进度社交域 · 层级/优先级：增强 / P2（元进度主轴）· 关联 F 码：F45（新增）· 关联：GDD §5.9（玩家等级系统）；SYSTEM_BREAKDOWN §S29
> 状态：v0.2-detailed · 日期 2026-07-17
> 设计基准：UI 750×1334（Cocos Creator 3.8.8 · 微信小游戏）· 安全区：顶部 y<88、底部 y>1290 不放置可点组件
> 数值约定：凡涉及 `xp_required` / `bonus` / 解锁等级 / 加成幅度的调优量为 `[PLACEHOLDER]`，标注「调优杆」，禁止硬编码魔法数字。
...
- **XP 来自每次战斗结算（S08）**：`xp_gain`（[PLACEHOLDER]）→ 累加 `current_xp` → 跨过 `xp_required` 阈值即升级。
...
| 字段 | 类型 | 取值/范围 | 默认值 | 说明 |
|---|---|---|---|---|
| level | int | 1–[PLACEHOLDER] | 1 | 等级（起始 1） |
| xp_required | int | 0–[PLACEHOLDER] | 0(L1) | **累计** XP 阈值（达到即为此级）。须严格递增。**调优杆**：升级节奏 |
| dmg_mult | float | 1.0–5.0 | 1.0 | 该级**全局塔伤害倍率（绝对值，非增量）**。**不累加** |
| range_mult | float | 1.0–3.0 | 1.0 | 该级全局射程倍率（绝对值）。**不累加** |
| atk_speed_mult | float | 1.0–3.0 | 1.0 | 该级全局攻速倍率（绝对值）。**不累加** |

**示例（CSV；数值列 `[PLACEHOLDER]` 为待调优占位，仅展示结构）**
...
> ⚠️ **不累加铁律在配置层的体现**：`dmg_mult` 等是**该级的总倍率**（绝对值快照），例如 L5 的 `dmg_mult=[P]` 表示"在 5 级时塔伤害 ×[P]"。**不要**写成"每级 +X%"的增量并求和。战斗内套用公式：
...
| required_level | int | 1–[PLACEHOLDER] | [PLACEHOLDER] | **解锁所需玩家等级（门槛）**。**调优杆**：解锁节奏 |
| unlock_type | enum | tower/meta_upgrade/system | tower | 解锁类型（决定写哪） |
| target_ref | string | tower_id / 加成键 / 系统 id | "t_magic" | 解锁对象 |
| effect_value | float | 类型相关 | 0 | meta_upgrade 数值（[PLACEHOLDER]） |

**示例（CSV；`required_level` 全 `[PLACEHOLDER]`，建议值见注释不写死）**

```csv
feature_id,feature_desc,required_level,unlock_type,target_ref,effect_value
unlock_magic,解锁魔法塔,[PLACEHOLDER],tower,t_magic,0
unlock_poison,解锁毒塔,[PLACEHOLDER],tower,t_poison,0
unlock_thunder,解锁电塔,[PLACEHOLDER],tower,t_thunder,0
unlock_gold,永久升级·起始金币+%,[PLACEHOLDER],meta_upgrade,start_gold_mult,[PLACEHOLDER]
unlock_lives,永久升级·漏怪容错+Lives,[PLACEHOLDER],meta_upgrade,leak_tolerance,[PLACEHOLDER]
unlock_wood,永久升级·木头产出+%,[PLACEHOLDER],meta_upgrade,wood_gain_mult,[PLACEHOLDER]
unlock_leaderboard,解锁排行榜,[PLACEHOLDER],system,S13,0
unlock_levels,解锁多关卡,[PLACEHOLDER],system,S14,0
```

> 注：`unlock_type=meta_upgrade` 的 3 项（start_gold / leak_tolerance / wood_gain）**刻意避开 dmg/range/atk_speed**，以免与 §3.1 等级加成（S29）在战斗基础属性上**双重计算**（见 §5 冲突说明）。原 S11 的 `dmg_global/range_global` 节点建议改为上述经济/容错类，详见 S11 修订。
...
- **S18 存档**：持久化 `player_level` / `current_xp` / 解锁态（**需 S18 在 `save_schema` 增加上述字段**，本次未改 S18，列为后续必做）。
...
### 5.3 冲突与一致性说明（务必关注）
1. **S29 等级加成 vs S11 原 `dmg_global/range_global` 升级节点（潜在双重计算）**：S11 原 meta 升级含"全局伤害+10%/射程+5%"，与 S29 每级 dmg/range 增益同为战斗基础属性增益，**若并存会双重计算**。本系统将 S29 等级加成定为 dmg/range/atk_speed（绝对值快照），并将 S11 的 `meta_upgrade` 节点**改为主题为经济/容错类**（start_gold / leak_tolerance / wood_gain），从语义上避免重叠。**需 Decision Owner 确认 S11 节点改主题**。
2. **S11 资源货币 TBD**：解锁改为等级门槛后，`meta_res` 的旧"解锁花费"语义失效；`meta_res` 是否保留/改用途（如纯展示或未来系统）**TBD**，已在 S11 标注。
3. **S18 存档 schema 需补字段**：`player_level`、`current_xp`、解锁态集合——本次 S29 未直接改 S18（不在交付清单），列为必跟更新项。
4. **GDD §5.x 编号**：胜负在 GDD 实为 §5.7，而 SYSTEM_BREAKDOWN/FEATURE_SCOPE 多处引用为 §5.9（**既有不一致**）；本系统顺延新增为 **§5.9 玩家等级系统**，与既有 §5.9 引用撞号，建议统一 GDD 胜负章节编号或修正外部引用（非本任务引入，提示修复）。
