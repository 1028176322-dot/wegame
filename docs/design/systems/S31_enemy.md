...
| 配置缺失 | `enemy_config[enemy_id]` 缺 | 用内置默认轻甲小怪（HP [PLACEHOLDER]） + 记 S25 | 可战不崩 |
...
| armor_type | enum | none/light/heavy/air/magic_immune/poison | — | 护甲（**套用 S30 枚举；S30 缺失，暂用 S04/S05 短式**，见依赖缺口声明） |
| weak_tower | enum | t_arrow/t_cannon/t_magic/t_ice/t_poison/t_electric/t_wind/all | — | 克制塔种（弱点；`all`=冰/风全克控制） |
| base_hp | int | 1–1000000 | `[PLACEHOLDER]` | 基础 HP（随 关卡 S14×波次 S04 缩放）。**调优杆**：难度 |
| move_speed | float | 0–500 | `[PLACEHOLDER]` | 移动速度(px/s)（随 关卡 S14×波次 S04 缩放）。**调优杆**：节奏 |
...
> | poison（毒甲/高HP肉盾） | t_poison（毒 DoT 越肉越赚） | 毒克高HP |
...
| wood_drop_rate | float | 0–1 | `[PLACEHOLDER]` | 掉木概率（session 木主源）。**调优杆**：养塔木供给节奏 |
| wood_drop_min | int | 1–999 | `[PLACEHOLDER]` | 掉木下限（个）。**调优杆** |
| wood_drop_max | int | 1–999 | `[PLACEHOLDER]` | 掉木上限（个）。**调优杆** |
| boss_extra_rate | float | 0–1 | `[PLACEHOLDER]` | Boss 额外掉率（叠加/覆盖，Boss 关键木点）。**调优杆** |
| boss_extra_min | int | 1–999 | `[PLACEHOLDER]` | Boss 额外掉木下限 |
| boss_extra_max | int | 1–999 | `[PLACEHOLDER]` | Boss 额外掉木上限 |
...
**多行示例数据（CSV；数值列 `[PLACEHOLDER]` 为待调优占位；枚举/字符串为实值）**
...
e_light_01,normal,史莱姆,light,t_arrow,[PLACEHOLDER],[PLACEHOLDER],none,spr_slime,false,null,drop_light
e_heavy_01,heavy,石巨人,heavy,t_cannon,[PLACEHOLDER],[PLACEHOLDER],none,spr_golem,false,null,drop_heavy
e_air_01,air,飞蝠,air,t_electric,[PLACEHOLDER],[PLACEHOLDER],air,spr_bat,false,null,drop_air
e_magic_immune_01,magic_immune,幽灵,magic_immune,t_arrow,[PLACEHOLDER],[PLACEHOLDER],magic_immune,spr_wraith,false,null,drop_magic_immune
e_boss_01,boss,巨龙,heavy,t_cannon,[PLACEHOLDER],[PLACEHOLDER],speedup,spr_dragon,true,speedup,drop_boss
```

```csv
drop_id,enemy_id,wood_drop_rate,wood_drop_min,wood_drop_max,boss_extra_rate,boss_extra_min,boss_extra_max
drop_light,e_light_01,[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],0,0,0
drop_heavy,e_heavy_01,[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],0,0,0
drop_air,e_air_01,[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],0,0,0
drop_magic_immune,e_magic_immune_01,[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],0,0,0
drop_boss,e_boss_01,[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER],[PLACEHOLDER]
```

> 说明：5 原型（普通轻甲/重甲/空军/魔免/Boss）为权威定义源；`poison` 护甲类型保留于枚举（高HP肉盾，弱 t_poison），供 S04 特定波引用。Boss 默认 `heavy` 护甲 + `speedup` 机制（可配 `heal_cut`）。所有 `[PLACEHOLDER]` 经试玩调优，禁止硬编码。
...
## 5. 一致性冲突与 NEEDS-DESIGN 清单

| # | 冲突/缺口 | 说明 | 处理 |
|---|---|---|---|
| A | **"魔法克魔免" 矛盾** | GDD §5.6 称魔法塔"魔免怪克星"，任务文本亦写"魔法克魔免"；但 `magic_immune` 字面 = 魔法伤害 ×0，且任务同时定义"魔免（魔法伤害×0）"。逻辑上魔免应被**物理**(arrow/cannon) 克制。 | S31 采用**自洽规则**：`magic_immune` 魔法伤害 ×0，弱点 = 物理塔。建议 GDD §5.6/§5.7 改为"物理克魔免"或重命名敌种。**NEEDS-DESIGN：GDD 措辞修正。** |
| B | **S30 属性系统缺失** | 任务要求"严格套用 S30 枚举/缩放因子"，仓库无 S30。 | 暂用 S04/S05/GDD 短式枚举 + S14/S04 缩放因子；armor_type 待 S30 建立后改为纯引用。**NEEDS-DESIGN：建 S30。** |
| C | **S32 关卡系统缺失** | 任务"HP/速度随关卡(S32)缩放"，仓库关卡实为 S14_level.md。 | 缩放引用 S14 `diff_mult`。**NEEDS-DESIGN：S32 与 S14 关系待裁定（疑似编号错位）。** |
| D | **S33 状态系统缺失** | 任务"状态叠加层接 S33 图标"，仓库无 S33。 | 图标引用 S05 §4 / S28 §4。**NEEDS-DESIGN：建 S33 或明确状态图标归 S05/S28。** |
| E | **enemy_config 归属冲突** | S16 §3.3 称 `enemy_config` 来自 S4；本任务定 S31 为唯一定义源。 | S31 为定义源，S04/S16 为消费者（只读）。S16 枚举长式 `light_armor` 须与本系统短式 `light` 对齐。**NEEDS-DESIGN：S16 契约枚举对齐。** |
| F | **掉木双 owner** | S04 已有波级 `drop_wood_*`；本系统新增逐敌 `enemy_drop`。 | 见 §3 结算规则（S04 存在则优先，否则用 enemy_drop）。**NEEDS-DESIGN：S04 后续委托本系统。** |
